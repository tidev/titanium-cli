					...
					options: Object.assign(
						{
							platform: {
								...
								callback(platform) {
									...
									p &&
										p.options &&
										Object.keys(p.options).forEach((name) => {
							...
							'project-dir': {
								...
								callback(projectDir) {
									...
									if (fs.existsSync(path.join(projectDir, 'tiapp.xml'))) {
										...
										const manifest = (cli.manifest = ti.loadModuleManifest(
											logger,
											path.join(projectDir, 'manifest')
										));
								...
								default: process.env.SOURCE_ROOT
									? path.join(process.env.SOURCE_ROOT, '..', '..')
									: '.',
								order: 1,
								prompt(callback) {
									callback(
								...
								validate(projectDir, callback) {
									...
									if (!isFound) {
										callback(
											new Error(
												`Invalid project directory "${projectDir}" because tiapp.xml or timodule.xml not found`
											)
										);
										return;
									}
...
exports.run = function run(logger, config, cli, finished) {
	...
	if (!fs.existsSync(buildModule)) {
		...
		logger.log(
			`Your SDK installation may be corrupt. You can reinstall it by running '${(cli.argv.$ + ' sdk install --force --default').cyan}'.\n`
		);
		process.exit(1);
	}
		...
		cli.env.getOSInfo(function (osInfo) {
			logger.log(
				[
					...
					'  ' +
						rpad('Target Platform') +
						' = ' +
						styleValue(ti.resolvePlatform(cli.argv.platform)),
					'',
