		...
		if (emus.length) {
			logger.log(emus.map(function (emu) {
				return '  ' + emu.name.cyan + '\n'
					...
					+ '  ' + rpad('  Google APIs')         + ' = ' + styleValue(emu.googleApis === null ? 'unknown, emulator not running' : emu.googleApis ? 'yes' : 'no');
			}).join('\n') + '\n');
		} else {
			logger.log('  ' + 'none'.grey + '\n');
		}
			...
			if (device.emulator) {
				switch (device.emulator.type) {
					...
					case 'genymotion':
						...
						result.push('  ' + rpad('Genymotion Version')  + ' = ' + styleValue(device.emulator.genymotion || 'unknown'));
						result.push('  ' + rpad('Display')             + ' = ' + styleValue(device.emulator.display || 'unknown'));
						result.push('  ' + rpad('DPI')                 + ' = ' + styleValue(device.emulator.dpi || 'unknown'));
						result.push('  ' + rpad('OpenGL Acceleration') + ' = ' + styleValue(device.emulator.hardwareOpenGL ? 'yes' : 'no'));
						result.push('  ' + rpad('Google APIs')         + ' = ' + styleValue(device.emulator.googleApis ? 'yes' : 'no'));
						break;
				}
