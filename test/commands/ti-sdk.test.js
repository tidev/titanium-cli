...
describe('ti sdk', () => {
	...
	describe('install', () => {
		it('should install an SDK and remove it', initSDKHome(async ({ run, tmpHome, tmpSDKDir }) => {
			...
			expect(output).toMatch(/Titanium Command-Line Interface/);
			expect(output).toMatch(new RegExp(`SDK Install Locations:\n\\s*${tmpSDKDir.replace(/\\/g, '\\\\')}`));
			...
			expect(output).toMatch(/Titanium Command-Line Interface/);
			expect(output).toMatch(new RegExp(`SDK Install Locations:\n\\s*${tmpSDKDir.replace(/\\/g, '\\\\')}`));
			expect(output).toMatch(new RegExp(`Installed SDKs:\n\\s*${sdkName}\\s+${sdkVersion}\\s+${
				sdkPath.replace(/\\/g, '\\\\')
			}`));
			expect(exitCode).toBe(0);
		...
		it('should error if SDK release not found', initSDKHome(async ({ run }) => {
			...
			expect(stderr).toMatch(/Unable to find any Titanium SDK releases or CI builds that match "foo"/);
			expect(exitCode).toBe(1);
		}));
	});
	...
	describe('list', () => {
		it('should list releases, branches, and builds', initSDKHome(async ({ run, tmpSDKDir }) => {
			...
			const json = JSON.parse(stdout);
			expect(json.branches.branches.includes('main') || json.branches.branches.includes('master')).toBe(true);
			expect(json.branches.branches.some(branch => /^\d+_\d+_X$/.test(branch))).toBe(true);
		...
		it('should list SDKs in SDK home directory', initMockSDKHome(async ({ run, tmpSDKDir }) => {
			...
			expect(output).toMatch(new RegExp(`0.0.0.GA\\s+0.0.0\\s+${join(tmpSDKDir, 'mobilesdk', os, '0.0.0.GA').replace(/\\/g, '\\\\')}`));
			expect(exitCode).toBe(0);
		}), 60000);
	});
