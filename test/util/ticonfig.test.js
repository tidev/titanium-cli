...
describe('TiConfig', () => {
	...
	it('should error load a bad config file', () => {
		expect(() => {
			...
			name: 'Error',
			message: expect.stringMatching(/^Unable to parse config file/)
		}));
	});
	...
	it('should error if file does not exist', () => {
		expect(() => {
			...
			name: 'Error',
			message: expect.stringMatching(/^Unable to open config file/)
		}));
	});
	...
	it('should error saving the file', async () => {
		...
		try {
			...
			name: 'Error',
			message: expect.stringMatching(/Unable to write config file/)
		}));
		} finally {
			await rmdir(dir);
		}
