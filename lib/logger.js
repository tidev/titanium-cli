...
logger.banner = function () {
	...
	if (bannerEnabled) {
		logger.log(info.about.name.cyan.bold + ', CLI v' + info.version + (logger.activeSdk ? ', Titanium SDK v' + logger.activeSdk : '') + '\n' + info.about.copyright);
		logger.log('\n' + __('Please report bugs to %s', 'https://github.com/tidev/titanium_mobile/issues'.cyan));
		logger.log(__('Browse the docs: %s', 'https://titaniumsdk.com'.cyan) + '\n');
