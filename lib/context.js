	...
	if (this.loaded) {
		...
		var conf = (typeof this.module.config == 'function' && this.module.config(logger, config, cli)),
			processConf = function processConf(conf) {
				...
				//
				// we do NOT want to fire flag/option callbacks here because if this is the
				// 'build' command, then --project-dir could throw a cli.GracefulShutdown
				// exception and fork the wrong ti command due to the args not being parsed fully.
				//
				// potential problem here because we are skipping callbacks during this parse.
				// if we're parsing for a command that has --platform, then we're ok, but
				// if it's for a command that doesn't have --platform (i.e. pretty much all non-
				// sdk commands), then option callbacks won't be fired.
				var argv = this.parse(cli.argv.$_, Object.keys(this.commands), true),
				...
				if (options.platform) {
					...
					if (!isHelpCommand && argv.platform) {
						...
						try {
							...
							if (ex instanceof cli.GracefulShutdown) {
								return;
							}
							throw ex;
						}
