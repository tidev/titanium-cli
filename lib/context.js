 ...
 * @private
 */
Context.prototype.setArg = function setArg(name, value) {
 ...
 */
Context.prototype.parse = function parse(args, commands) {
	var argv = this.argv,
		i, len, arg, name, next, letters, broken;

	// Since argv is shared across multiple parse() calls, we need to reset it each
	// time we parse. Note that all unknown flags are treated as options (provided
	// there is a value present). This could cause interesting scenarios like when
	// you call "ti --build-only build --target device".
	this.argv._ = [];
