...
function Context(params) {
	...
	if (params && params.conf) {
		params.conf.flags && this.flag(params.conf.flags);
		params.conf.options && this.option(params.conf.options);
	}
}
		...
		var conf = (typeof this.module.config == 'function' && this.module.config(logger, config, cli)),
			processConf = function processConf(conf) {
				...
				function addAuthOptions() {
					...
					obj.conf || (obj.conf = {});
					obj.conf.options || (obj.conf.options = {});
					obj.conf.options.username || (obj.conf.options.username = {
						default: config.user && config.user.email,
						desc: __('user to log in as, if not already logged in')
					});
					obj.conf.options.password || (obj.conf.options.password = {
				...
				this.requireAuth && addAuthOptions.call(this);
				...
				conf.platforms && Object.keys(conf.platforms).forEach(function (platform) {
					var platformConf = conf.platforms[platform],
						platformContext = this.platforms[platform] = new Context({
							title: platformConf.title || platform,
							name: platform,
							path: path.join(cli.sdk.path, platform),
							parent: this
						});
					platformContext.conf = platformConf;
					platformConf.flags && platformContext.flag(platformConf.flags);
					platformConf.options && platformContext.option(platformConf.options);
					platformConf.args && (platformContext.args = platformConf.args);
					platformConf.subcommands && platformContext.subcommand(platformConf.subcommands);
				...
				if (cli.argv) {
					...
					// check if platform was passed in
					if (argv.platform) {
						this.platform = this.platforms[argv.platform];

						mix(argv, this.platform.parse(cli.argv.$_, [this.name]));
						argv.$platform = argv.platform;

						// find all platform hooks
						cli.scanHooks(path.join(cli.sdk.path, this.platform.name, 'cli', 'hooks'));

						// TODO: add support for platform level subcommands
					} else {
...
Context.prototype.parse = function parse(args, commands, skipCallbacks) {
	...
	if (this.parent) {
		...
		this.flag(parentFlagsOptions.flags, true, true);
		this.option(parentFlagsOptions.options, true, true);
	}
		...
		parallel(this, Object.keys(this.commands).map(function (name) {
			...
			return function (next) {
				if (cmd.loaded) {
					...
					cmd.load(logger, config, cli, function (err) {
