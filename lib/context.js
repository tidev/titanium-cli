...
Context.prototype.printHelp = function printHelp(logger, config, cli, command, subcommand, finished) {
	if (command) {
//console.log('global context printing command context ---------------------------------------------------------------------------------------------');
		...
		parallel(this, [
			...
			//this.printSubcommands(logger);
			commandContext.printHelp(logger, config, cli, null, subcommand, function () {
//console.log('global context callback ---------------------------------------------------------------------------------------------------------------------');
//dump(this.conf);
				// print global flags and options
				this.printArguments(logger);
...
//console.log('command context ---------------------------------------------------------------------------------------------------------------------');
		if (subcommand) {
			dump(this.conf.subcommands[subcommand]);
		} else {
//dump(this.conf);
			this.printSubcommands(logger);
			this.printArguments(logger);
...
Context.prototype.printSubcommands = function printSubcommands(logger) {
	if (this.conf.subcommands && Object.keys(this.conf.subcommands).length) {
...
Context.prototype.printArguments = function printArguments(logger, config) {
	if (this.conf && this.conf.args && Object.keys(this.conf.args).length) {
		this.printList(logger, __('%s Arguments:', this.title || '').trim(), this.conf.args.filter(function (arg) {
...
Context.prototype.printFlags = function printFlags(logger) {
	if (this.conf && this.conf.flags && Object.keys(this.conf.flags).length) {
		this.printList(logger, __('%s Flags:', this.title || '').trim(), Object.keys(this.conf.flags).sort().map(function (name) {
...
Context.prototype.printOptions = function printOptions(logger, config) {
	if (this.conf && this.conf.options && Object.keys(this.conf.options).length) {
		this.printList(logger, __('%s Options:', this.title || '').trim(), Object.keys(this.conf.options).sort().map(function (name) {
