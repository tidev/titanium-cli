		...
		parallel(this, Object.keys(this.commands).map(function (name) {
			...
			return function (next) {
				if (cmd.loaded) {
					...
					cmd.load(logger, config, cli, function (ex) {
						// note: it's possible that the command failed to load, so we just
 ...
 * @private
 */
Context.prototype.printUsage = function printUsage(logger, config, cli) {
	if (this.parent) {
		var width = config.get('cli.width', process.stdout.columns || 80),
			padding = (new Array(8)).join(' '), // padding for "Usage: "
			parts = [ cli.argv.$ ];
		...
		function doFlags(conf) {
			if (conf && conf.flags && Object.keys(conf.flags).length) {
				Object.keys(conf.flags).sort().forEach(function (name) {
					...
					if (!flag.hidden) {
						parts.push((flag.required ? '' : '[') + (flag.negate ? '--no-' + name : '--' + name) + (flag.required ? '' : ']'));
					}
				}.bind(this));
			}
		}
		...
		function doOptions(conf) {
			if (conf && conf.options && Object.keys(conf.options).length) {
				Object.keys(conf.options).sort().forEach(function (name) {
					...
					if (!opt.hidden) {
						parts.push((opt.required ? '' : '[') + '--' + name + ' <' + hint + '>' + (opt.required ? '' : ']'));
					}
				}.bind(this));
			}
		}
		...
		if (this.parent.parent) {
			...
			parts.push(this.parent.name, this.name);
			...
			// subcommands
			this.subcommands && Object.keys(this.subcommands).length && parts.push('<subcommand>');
		...
		logger.log(__('Usage') + ': ' + string.wrap(parts.join(' '), width - padding.length).split('\n').map(function (line) { return line.trim(); }).join('\n' + padding).cyan + '\n');
	} else {
		// global
...
Context.prototype.printArguments = function printArguments(logger, config) {
	if (this.conf && this.conf.args && Object.keys(this.conf.args).length) {
		this.printList(logger, config, __('%s Arguments:', (this.parent && this.parent.parent ? this.parent.title + ' ' : '') + (this.title || '')).trim(), this.conf.args.filter(function (arg) {
...
Context.prototype.printFlags = function printFlags(logger, config) {
	if (this.conf && this.conf.flags && Object.keys(this.conf.flags).length) {
		this.printList(logger, config, __('%s Flags:', (this.parent && this.parent.parent ? this.parent.title + ' ' : '') + (this.title || '')).trim(), Object.keys(this.conf.flags).sort().map(function (name) {
...
Context.prototype.printOptions = function printOptions(logger, config) {
	if (this.conf && this.conf.options && Object.keys(this.conf.options).length) {
		this.printList(logger, config, __('%s Options:', (this.parent && this.parent.parent ? this.parent.title + ' ' : '') + (this.title || '')).trim(), Object.keys(this.conf.options).sort().map(function (name) {
