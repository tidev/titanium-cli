	...
	this.emit('cli:pre-validate', { cli: this, command: this.command }, function () {
		...
		series(this, [
			function (next) {
				...
				async.series(Object.keys(options).map(function (name) {
					return function (cb) {
			...
			function (next) {
				...
				async.series(Object.keys(invalidOptions).map(function (name) {
					return function (cb) {
						...
						if (opt.prompt && typeof opt.prompt == 'function') {
							...
							// old style, but we can use a select list
							set(fields.select({
							...
							// old style and we need to use a text prompt
							var pr = opt.prompt || {};
							set(fields.text({
				...
				if (appc.version.gte(this.sdk.manifest.version, '3.2.0')) {
					...
					async.whilst(
						...
						function (callback) {
							...
							name = names.shift();
							if (name) {
								this.prompt(missing[name], callback);
								return;
							}
