		...
		series(this, [
			function handleMissingAndInvalidOptions(nextValidationTask) {
				...
				if (appc.version.gte(this.sdk.manifest.version, '3.2.0')) {
					...
					// we use an async while loop to process each missing option. As
					// soon as an invalid or missing option has been prompted for, we
					// recompute the invalid and missing options just in case an invalid
					// or missing option (such as --target) adds new dependencies.

					var _t = this,
						done = false,
						prompting = this.config.cli.prompt,
					...
					function wireupPrePrompt(field, opt) {
					...
					async.whilst(
						...
						function (callback) {
							...
							appc.async.series(this, orderedOptionNames.map(function (name) {
								// if we're prompting, we only need to find the first invalid one
								if (prompting && invalidCount) return;
								...
								if (prompting) {
									...
									if (invalidCount) {
										...
										if (opt.prompt) {
											...
											opt.prompt = function (callback) {
												fn(function (field) {
													callback(wireupPrePrompt(field, opt));
												});
											};
										} else {
									...
									// if we're not prompting, just output the error message right away
									this.logger.banner();
					...
					// TODO: need to determine missing and invalid options!!!!!!!!!!
					console.log('TODO: need to determine missing and invalid options!!!!!!!!!!');
					process.exit(1);
