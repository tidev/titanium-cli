...
cli.parse = function (finished) {
	...
	if (args.length) {
		// process the args
		processArg(args, 0, globalFlags, appc.util.hitch(this, done));
	} else {
		done.call(this);
	}
};
...
cli.validate = function (callback) {
	...
	// if we have a valid command, process the options and check the sdk
	if (command) {
	...
	var missingOptions = [],
		...
		scanOptions = function (opts) {
			opts && Object.keys(opts).forEach(function (name) {
				if ((!argv.hasOwnProperty(name) || argv[name] == void 0 || argv.$defaulted[name]) && (!opts[name].platform || opts[name].platform == platform)) {
					var obj = appc.util.mix({ name: name }, opts[name]);
					if (opts[name].required) {
						argv.$defaulted[name] && (obj.defaulted = true);
						missingOptions.push(obj);
					}
					opts[name].values && !opts[name].skipValueCheck && valuedOptions.push(obj);
				}
			});
		};
	...
	if (command) {
		...
		if (sdk) {
			Object.keys(command[sdk.name]).forEach(function (platform) {
				checkArgumentNames(command[sdk.name][platform]);
			});
	...
	async.series([
		createPrompter(missingOptions, 'Missing required option "--%s"'),
		createPrompter(missingArguments, 'Missing required argument "%s"')
...
function createPrompter(missing, errorMsg) {
	return function (next) {
		...
		if (!config.cli.prompt) {
			missing.forEach(function (m) {
				!m.defaulted && tierror(__(errorMsg, m.name));
			});
			console.log('');
			process.exit(1);
		}
...
function setArg(key, val, gflag, isDefault) {
	var keys = key.split('.'),
		num = Number(val),
	...
	if (isDefault) {
		obj.$defaulted[key] = obj[key];
	} else {
		delete obj.$defaulted[key];
	}
	...
	(aliases[key] || []).forEach(function (x) {
		...
		x.forEach(function (y) {
			obj[y] = obj[key];
			isDefault && (obj.$defaulted[y] = obj[key]);
			if (isDefault) {
				obj.$defaulted[y] = obj[key];
			} else {
				delete obj.$defaulted[y];
			}
		});
	});
...
function processConf(ctx, sdk, platform, conf) {
	...
	if (!conf.noAuth && (!status.loggedIn || status.expired)) {
	...
	conf.options && Object.keys(conf.options).forEach(function (name) {
		...
		opt.hasOwnProperty('default') && setArg(name, opt.default, null, true);
	});
...
function processArg(args, i, globalFlags, finished) {
	var arg = args[i];
	
	function nextArg() {
		if (++i < args.length) {
			processArg(args, i, globalFlags, finished);
		} else {
			finished();
		}
	}
	...
	if (arg === '--') {
		...
		var m = arg.match(/^(?:--|Ñ)([^=]+)=(.*)/);
		setArg(m[1], m[2]);
	} else if (arg.match(/^(?:--|Ñ)no-.+/)) {
		// --no-flag
		var key = arg.match(/^(?:--|Ñ)no-(.+)/)[1];
		setArg(key, false, globalFlags[key]);
	} else if (arg.match(/^(?:--|Ñ).+/)) {
		// --flag or --option
		var key = arg.match(/^(?:--|Ñ)(.+)/)[1],
			next = args[i + 1];
			...
			if (!cli.argv.$command) {
				...
				async.parallel(Object.keys(c).map(function (sdk) {
					return function (callback) {
						if (sdk === '__global__') {
							loadCommand(c[sdk], sdk, null, nextArg);
						} else {
				...
				match && (cli.argv.$subcommand = cli.argv._[0]);
			}
		}
	}
