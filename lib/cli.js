...
cli.flag = cli.flags = createFunction('flags');

cli.option = cli.options = createFunction('options');
...
cli.parse = function (finished) {
	...
	if (args.length) {
		// process the args
		processArg(argv, args, 0, appc.util.hitch(this, done));
	} else {
		done.call(this);
	}
};
		...
		function checkArgumentNames(ctx) {
			ctx.args && ctx.args.forEach(function (arg, i) {
				...
				if (i < argv._.length) {
					...
					argv._.push(argv[arg.name] = arg.default || '');
				}
			});
		}
...
function processArg(argv, args, i, finished) {
	var arg = args[i];
	
	function nextArg() {
		if (++i < args.length) {
			processArg(argv, args, i, finished);
		} else {
			finished();
		}
	}
