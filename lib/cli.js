	...
	var missing = [],
		...
		scanOptions = function (opts) {
			opts && Object.keys(opts).forEach(function (name) {
				...
				opts[name].required && !argv.hasOwnProperty(name) && missing.push(obj);
				opts[name].values && !opts[name].skipValueCheck && !argv.hasOwnProperty(name) && valued.push(obj);
			});
		};
	...
	if (missing.length) {
		if (config.cli.prompt) {
			...
			missing.forEach(function (m) {
				var p = (m.opt.promptLabel || appc.string.capitalize(m.opt.desc)).trim().replace(/\:$/, ''),
					pr = m.opt.prompt || {};
				schema.properties[m.name] = {
					conform: pr.validator,
					default: m.opt.default || (m.opt.values ? m.opt.values.join(',') : void 0),
					errorMsg: pr.error,
