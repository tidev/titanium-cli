...
function CLI(params) {
	...
	// handy flag if this command/subcommand requires authentication
...
CLI.prototype.flag = function flag(name, params) {
	this.globalContext.flag(name, params);
	return this;
};
...
CLI.prototype.option = function flag(name, params) {
	this.globalContext.option(name, params);
	return this;
};
...
CLI.prototype.parseGlobalArgs = function parse(next) {
	this.emit('cli:global-args-parsed', {
		cli: this,
		args: mix({}, this.globalContext.parse(this.argv.$_), this.argv)
	}, next);
}
...
CLI.prototype.loadCommand = function loadCommand(next) {
	...
	this.argv = mix(this.argv, this.globalContext.parse(this.argv.$_, Object.keys(commands), true));
...
CLI.prototype.configureCommand = function configureCommand(next) {
	var commandConf = this.command.conf || (this.command.conf = {});

	this.requireAuth = !commandConf.noAuth && !appc.auth.status().loggedIn;

	function addAuthOptions() {
		commandConf.options || (commandConf.options = {});
		commandConf.options.username && (commandConf.options.username = {
			default: config.user && config.user.email,
			desc: __('user to log in as, if not already logged in')
		});
		commandConf.options.password || (commandConf.options.password = {
	...
	// add this command to the global context
	var commandContext = this.globalContext.command(this.command);

	// parse the args in the returned command context
	var argv = commandContext.parse(this.argv.$_, Object.keys(this.commands));
	...
	if (Object.keys(subcommands).length) {
		var subcmd = argv.$subcommand = argv._.length && argv._[0] || this.command.conf.defaultSubcommand;
		if (subcommandContext = subcommands[subcmd]) {
			// if the subcommand requires authentication, add the username
			// and password options
			this.requireAuth = this.requireAuth || !commandConf.subcommands[subcmd].noAuth;
			this.requireAuth && addAuthOptions();

			// parse the args again to get any subcommand flags/options
			var sargv = subcommandContext.parse(this.argv.$_, this.commands.name, Object.keys(subcommands));

			// after parsing, we need to remove the command and subcommand
			sargv._.splice(0, 2);

			mix(this.argv, argv, subcommandContext.applyArgs(sargv._));

			isSubcommand = true;
		}
	}

	subcommandContext || mix(this.argv, argv, commandContext.applyArgs(argv._));
