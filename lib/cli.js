...
cli.parse = function (finished) {
	...
	if (args.length) {
		// process the args
		processArg(args, 0, appc.util.hitch(this, done));
	} else {
		done.call(this);
	}
};
...
function setArg(key, val, isDefault) {
	var keys = key.split('.'),
		num = Number(val),
	...
	if (alias) {
		argv[alias] = argv[key];
		key = alias;
		isDefault && (argv.$defaulted[alias] = argv[key]);
		if (isDefault) {
			argv.$defaulted[alias] = argv[key];
		} else {
	...
	flags && flags[key] && flags[key].callback && flags[key].callback(val, logger);
}
...
function processArg(args, i, finished) {
	...
	if (arg === '--') {
		...
		var m = arg.match(/^(?:--|A)([^=]+)=(.*)/);
		setArg(m[1], m[2]);
	} else if (arg.match(/^(?:--|A)no-.+/)) {
		// --no-flag
		var key = arg.match(/^(?:--|A)no-(.+)/)[1];
		setArg(key, false);
	} else if (arg.match(/^(?:--|A).+/)) {
		// --flag or --option
		var key = arg.match(/^(?:--|A)(.+)/)[1],
			next = args[i + 1];
