	...
	if (!this.scannedCommandPaths[dir] && afs.exists(dir)) {
		...
		(isDir ? fs.readdirSync(dir) : [dir]).forEach(function (filename) {
			...
			if (fs.statSync(file).isFile() && jsfile.test(filename) && (!isDir || !ignore.test(path.basename(file)))) {
				...
				this.commands[name] || (this.commands[name] = file);
			}
		}, this);
...
CLI.prototype.loadCommand = function loadCommand(next) {
	...
	this.argv = mix(this.argv, this.globalContext.parse(this.argv.$_, Object.keys(this.commands), true));
	...
	var desiredCommand = this.argv.$command = this.argv._[0],
		cmdPath = this.commands[desiredCommand];

	// check if the desired command exists
	if (!cmdPath) {
		this.logger.banner();
		this.logger.error(__('"%s" is an unrecognized command.', desiredCommand) + '\n');
		appc.string.suggest(desiredCommand, Object.keys(this.commands), this.logger.log);
...
CLI.prototype.configureCommand = function configureCommand(next) {
	...
	this.requireAuth = !commandConf.noAuth;
	...
	if (Object.keys(subcommands).length) {
		var subcmd = argv._.length && argv._[0] || this.command.conf.defaultSubcommand;
		if (subcommandContext = subcommands[subcmd]) {
			...
			argv = subcommandContext.parse(this.argv.$_, Object.keys(subcommands).concat(this.command.name));
...
CLI.prototype.login = function login(next) {
	...
	if (this.requireAuth && !appc.auth.status().loggedIn) {
		var argv = this.argv,
