		...
		async.series(pres.map(function (pre) {
			...
			var next = function (result) {
				...
				async.series(posts.map(function (post) {
					return function (cb) {
						post.call(null, data, function (e) {
							e && (data = e);
							cb();
						});
					};
...
cli.fireHook = function (name, callback) {
...
cli.scanHooks = function (dir) {
	if (!hooks.scannedPaths[dir] && afs.exists(dir)) {
		fs.readdirSync(dir).forEach(function (file) {
			...
			if (fs.statSync(file).isFile() && /\.js$/.test(file) && !/\.?_.*| |\.DS_Store/.test(path.basename(file))) {
				var mod = require(file);
				mod.init && mod.init(logger, config, cli);
			}
		});
	}
...
cli.parse = function (finished) {
	...
	if (args.length) {
		// process the args
		processArg(args, 0, hitch(this, done));
	} else {
		done.call(this);
	}
};
		...
		if (ctx.options) {
			...
			Object.keys(ctx.options).forEach(function (name) {
				var opt = ctx.options[name],
					obj;
				if ((!argv.hasOwnProperty(name) || argv[name] == void 0 || argv.$defaulted[name]) && (!opt.platform || opt.platform == argv.platform)) {
		...
		if (typeof name == 'object') {
			...
			if (params.alias) {
				...
				params.alias.forEach(function (alias) {
					if (aliases[alias]) {
						tierror('Already have an alias for "' + alias + '" that maps to "' + aliases[alias] + '"');
					}
					aliases[alias] = name;
				});
			}
			...
			if (params.abbr) {
				if (aliases[params.abbr]) {
					tierror('Already have an alias for "' + params.abbr + '" that maps to "' + aliases[params.abbr] + '"');
				}
...
function setArg(key, val, isDefault) {
	...
	if (alias) {
		argv[alias] = argv[key];
		isDefault && (argv.$defaulted[alias] = argv[alias]);
		if (isDefault) {
		...
		if (cli.argv._.length == 1) {
			if (!cli.argv.$command) {
				...
				Object.keys(command).forEach(function (sdk) {
					if (sdk === '__global__') {
						loadSubcommand(command[sdk], sdk);
					} else {
						Object.keys(command[sdk]).forEach(function (platform) {
							loadSubcommand(command[sdk][platform], sdk, platform);
						});
					}
				});
