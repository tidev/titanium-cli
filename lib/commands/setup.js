			...
			'network': function (next) {
				...
				appc.net.online(function (err, online) {
					...
					*/
					...
					, function() {
						// test URI access via cURL
/*
// curl test disabled so that tickets can be addressed individually
						var curlArgs = [
							config.get('cli.httpProxyServer'),
							'-sS',
							'-d'
							];
						var outObj;
						appc.subprocess.findExecutable(config.get('executables.curl', 'curl') + (process.platform === 'win32' ? '.exe' : ''), function (err, executable) {
							if (err) {
								// skip curl tests
							} else {
								appc.subprocess.run(executable, curlArgs.concat(['un=unit_tests@aptana.com&pw=unittestSSSSSSSS&mid=studio', 'https://api.appcelerator.net/p/v1/sso-login']), function (code, out, err) {
									// curl test passes if `code` is falsey and out is JSON parsed and successful
									outObj = (out) ? JSON.parse(out) : {};
									if(err || !code) {
										if(outObj.reason === 4) {
											r.curl.push('api.appcelerator.net (invalid password)');
										} else {
											r.curl.push('api.appcelerator.net (' + outObj.reason + ')');
										}
									}
								});
							}
						...
						appc.subprocess.findExecutable(config.get('executables.curl', 'curl') + (process.platform === 'win32' ? '.exe' : ''), function (err, executable) {
							if (err) {
								...
								appc.subprocess.run(executable, curlArgs.concat(['username=unit_tests@aptana.com&password=unittestSSSSSSSS&mid=studio', 'https://dashboard.appcelerator.com/api/v1/auth/login']), function (code, out, err) {
									// curl test passes if `code` is falsey and out is JSON parsed and successful
									outObj = (out) ? JSON.parse(out) : {};
									if(err || !code) {
										if(outObj.code === 400) {
											r.curl.push('dashboard.appcelerator.com (invalid password)');
										} else {
											r.curl.push('dashboard.appcelerator.com (' + outObj.description + ')');
										}
									}
								});
							}
...
/*
// java test disabled so that tickets can be addressed individually
						var command = "java ",
							jarPath,
							hiddenText;
						if (config.get('cli.httpProxyServer')) {
							var proxyParts = config.get('cli.httpProxyServer').split(':');
							if(proxyParts && proxyParts.length && proxyParts.length === 2) {
								command += "-D" + proxyParts[0] + ".proxyHost=" + proxyParts[1].replace('\/\/', '') + " ";
							}
							if(proxyParts && proxyParts.length && proxyParts.length > 2) {
								command += "-D" + proxyParts[0] + ".proxyPort=" + proxyParts[2] + " ";
							}
						}
						...
						exec(command, function (error, stdout, stderr) {
							try {
								...
								r.javaResults.push(err.message);
							}
						});
*/
						next(null, r);
...
/*
// curl tests disabled so that tickets can be address separately
					if (r.curl.length && r.curl.length > 1) {
						r.curl.forEach(function(unreachableUrl) {
							if(unreachableUrl.indexOf('Unexpected end of input') === -1) {
								// Omitting "Unexpected end of input" from the output
								bad(unreachableUrl + __(' is unreachable via cURL'));
							}
						});
					} else {
						ok(__('cURL connection test'));
					}
*/
/*
// java test disabled so that tickets can be addressed separately
					if (r.javaResults.length) {
						r.javaResults.forEach(function(javaURL) {
							bad(javaURL + __(' is unreachable via Java'));
						});
					} else {
						ok(__('Java connection test'));
					}
*/
				} else {
					note(__('Network connection test'));
//					note(__('cURL connection test'));
//					note(__('Java connection test'));
				}
				log();
			}(results.network));
