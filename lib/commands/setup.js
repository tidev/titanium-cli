...
SetupScreens.prototype.mainmenu = function mainmenu(callback) {
	...
	fields.select({
		...
		].filter(function (o) { return process.platform == 'darwin' || o.value != 'ios'; })
	}).prompt(callback);
};
...
SetupScreens.prototype.check = function check(callback) {
	...
	async.parallel({
				...
				request({
					...
					if (!error && response.statusCode == 200) {
						for (i = 0, vers = body.split('\n'), l = vers.length; i < l; i++) {
							...
							if (nodeVer.split('.')[1] % 2 != 1 && appc.version.gt(nodeVer, r.node.latest) && appc.version.lt(nodeVer, '0.11')) {
		...
		function note(label, status, extra) {
			log('  ' + '-'.bold.grey + '  ' + appc.string.rpad(label, labelPadding) + ' ' + (status ? status.grey : '') + (extra ? ' ' + extra.grey : ''));
		}
		...
		process.platform == 'darwin' && (function (r) {
			...
			if (r.xcodeCLITools) {
				...
				Object.keys(r.xcode).forEach(function (ver) {
					if (ver != '__selected__') {
						r.xcode[ver].cliTools || missing.push(r.xcode[ver].version);
					}
				});
				if (missing.length) {
					warn(appc.string.rpad(__('CLI Tools'), len), __('not found for Xcode %s', missing.join(', ')));
				} else {
					ok(appc.string.rpad(__('CLI Tools'), len), __('installed'));
				}
			} else {
				note(appc.string.rpad(__('CLI Tools'), len), __('select Titanium SDK 3.2 or newer to detect'));
			}
				...
				if (info[0]) {
					...
					if (fs.existsSync(dir)) {
						if (afs.isDirWritable(dir)) {
							...
							bad(info[1], __('"%s" not writable', dir));
						}
					}
				}
			});
