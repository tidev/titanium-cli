				...
				appc.net.online(function (err, online) {
					...
					*/
					...
					, function() {
						...
						exec("curl" + config.get('cli.httpProxyServer') + " -d \"un=unit_tests@aptana.com&pw=unittestSSSSSSSS&mid=studio\" https://api.appcelerator.net/p/v1/sso-login -sS -f", function (error, stdout, stderr) {
							try {
						...
						exec("curl" + config.get('cli.httpProxyServer') + " -d \"username=unit_tests@aptana.com&password=unittest&from=studio\" https://dashboard.appcelerator.com/api/v1/auth/login -sS -f", function (error, stdout, stderr) {
							try {
								result = JSON.parse(stdout);
								if(!result.success) {
