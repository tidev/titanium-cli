...
SetupScreens.prototype.check = function check(callback) {
	...
	async.parallel({
				...
				request({
					...
					if (!error && response.statusCode == 200) {
						for (i = 0, vers = body.split('\n'), l = vers.length; i < l; i++) {
							...
							if (nodeVer.split('.')[1] % 2 != 1 && appc.version.gt(nodeVer, r.node.latest)) {
								r.node.latest = nodeVer;
								r.npm.latest = npmVer;
							}
						}
					}
							...
							Object.keys(pkginfo.dependencies).forEach(function (name) {
								...
								if (fs.existsSync(p)) {
									try {
										...
										results[name].latest = pkginfo.version;
									} catch (ex2) {}
								}
							});
						} catch (ex) {}
		...
		'tisdk': function (next) {
			request({
				url: 'http://api.appcelerator.net/p/v1/release-list',
				proxy: config.get('cli.httpProxyServer')
			}, function (error, response, body) {
				var results = {
		...
		function ok(label, status, extra) {
			log('  ✓'.green + '  ' + appc.string.rpad(label, labelPadding) + status.green + (extra ? ' ' + extra.grey : ''));
		}

		function warn(label, status, extra) {
			log('  !'.yellow.bold + '  ' + appc.string.rpad(label, labelPadding) + status.yellow + (extra ? ' ' + extra.grey : ''));
		}

		function bad(label, status, extra) {
			log('  \u2715'.red + '  ' + appc.string.rpad(label, labelPadding) + status.red + (extra ? ' ' + extra.grey : ''));
		}

		function update(label, status, extra) {
			log('  \u2605'.magenta + '  ' + appc.string.rpad(label, labelPadding) + status.magenta + (extra ? ' ' + extra.grey : ''));
		}

		(function (r) {
			log('Node.js');
			if (r.node.current == r.node.latest) {
		...
		(function (r) {
			log('Titanium SDK');
			if (!r.current) {
				bad('latest sdk', __('no installed Titanium SDKs found'));
			} else if (appc.version.gt(r.latest, r.current)) {
				update('latest sdk', __('new version v%s available!', r.latest), __('(currently v%s)', r.current));
			} else {
				ok('latest sdk', __('up-to-date'), '(v' + r.current + ')');
			}
			...
			if (r.current) {
				if (selectedSdk) {
					...
					if (appc.version.gte(selectedSdk, r.current)) {
						ok('selected sdk', __('up-to-date'),'(v' + r.current + ')');
					} else {
						warn('selected sdk', __('latest Titanium SDK "v%s" is not the selected SDK', r.latest), __('(currently v%s)', selectedSdk));
					}
				} else if (!cli.env.sdks[selectedSdk]) {
					// bad, invalid selected sdk, select an sdk
					bad('selected sdk', __('selected Titanium SDK "v%s" is not installed', selectedSdk));
				} else {
					// bad, no selected sdk, select an sdk
					bad('selected sdk', __('no selected Titanium SDK'));
				}
			} else {
				bad('selected sdk', __('no installed Titanium SDKs found'));
			}
			log();
		}(results.tisdk));
