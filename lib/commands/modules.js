...
var appc = require('node-appc'),
	mix = appc.util.mix,
	timodule = appc.timodule,
	async = require('async');

exports.title = __('Modules');
...
function list(logger, argv, env) {
	async.parallel([
		function (next) {
			timodule.getGlobalModules(function (modules) {
				next(null, modules);
			});
		},
		function (next) {
			timodule.getProjectModules(argv['project-dir'], function (modules) {
				next(null, modules);
			});
		}
		...
		if (argv.output == 'json') {
			...
			Object.keys(results).sort().forEach(function (name) {
				...
				Object.keys(results[name]).sort().forEach(function (version) {
					...
					logger.log('   Platforms        = ' + Object.keys(info.platforms).sort().join(', ').cyan);
					/*
	...
	async.parallel([
		...
		if (globalResults[module]) {
			if (globalResults[module][version]) {
				timodule.activateGlobalModule(module, version);
			...
			if (projectResults[module][version]) {
				timodule.activateProjectModule(argv['project-dir'], module, version);
				logger.log(__('Module %s version %s was activated successfully', module.cyan, version.cyan) + '\n');
			} else {
				logger.error(__('Invalid module version %s', version) + '\n');
				process.exit(1);
...
function deactivate(logger, argv, env) {
	async.parallel([
		...
		logger.banner();
		if (globalResults[module]) {
			timodule.deactivateGlobalModule(module);
			logger.log(__('Module %s was deactivated successfully', module.cyan) + '\n');
		} else if (projectResults[module]) {
			timodule.deactivateProjectModule(argv['project-dir'], module);
			logger.log(__('Module %s was deactivated successfully', module.cyan) + '\n');
		} else {
			logger.error(__('Unknown module %s', module) + '\n');
			process.exit(1);
