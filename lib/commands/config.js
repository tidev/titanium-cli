	...
	if (key) {
		try {
			if (value) {
				// doing a set
				key = key.split('.');
				if (key.length === 2 && key[0] === 'paths') {
					if (!~['modules', 'commands', 'hooks', 'modules'].indexOf(key[1])) {
						logger.error(__('Unsupported key %s', key.join('.')));
						return;
					}
					config.paths || (config.paths = {});
					config.paths[key[1]] || (config.paths[key[1]] = []);
					~config.paths[key[1]].indexOf(value) || (config.paths[key[1]].push(value));
					key = key.join('.');
				} else {
