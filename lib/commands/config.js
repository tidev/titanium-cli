		...
		try {
			if (value) {
				...
				if (listMatch) {
					...
					if (['commands', 'hooks', 'modules', 'plugins', 'sdks'].indexOf(subPath) == -1) {
				...
				if (obj) {
					if (argv.remove) {
						...
						if (Object.prototype.toString.call(obj[q]) == '[object Object]') {
							...
							if (obj[q].length) {
								logger.log(asJson ? JSON.stringify(obj[q]) : obj[q].join('\n'));
							}
						} else if (obj[q] != void 0) {
							obj[q] && logger.log(asJson ? JSON.stringify(obj[q]) : obj[q]);
						} else {
							logger.log(asJson ? JSON.stringify({
								success: false,
								reason: __('key "%s" not found', key)
