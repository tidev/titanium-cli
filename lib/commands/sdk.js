...
SdkSubCommands.list = {
	...
	fn: function list(logger, config, cli, finished) {
		...
		cli.argv.branches && (tasks.branches = function (next) {
			fetch(urls.branches, config, next);
		});
...
SdkSubCommands.install = {
	...
	fn: function install(logger, config, cli, finished) {
		...
		async.series([
			...
			var setDefault = (data.setDefault || cli.argv.default) && appc.version.gte(data.version, '3.0.0') && (cli.argv.default || appc.version.gt(data.version, config.app.sdk)),
				analyticsPayload = {};
	...
	fn: function uninstall(logger, config, cli, finished) {
		...
		async.series([
			...
			if (config.app.sdk == version) {
				// need to force the config to reload
				config.load();
				config.app.sdk = vers.shift() || 'latest';
				config.save();
...
function fetch(url, config, callback) {
	request({
		...
		if (error) {
			...
			callback(new Error(__('Request failed with HTTP status code %s %s', req.statusCode, http.STATUS_CODES[req.statusCode] || '')));
		} else {
			try {
