...
function sortVersion(a, b) {
	const [ , amajor, aminor, apatch, atag ] = a.name.toLowerCase().match(versionRegExp);
	const [ , bmajor, bminor, bpatch, btag ] = b.name.toLowerCase().match(versionRegExp);
...
async function install(logger, config, cli) {
	...
	try {
		...
		if (file !== zipFile) {
			tempDir = tmp.tmpNameSync({ prefix: 'titanium-' });
		}
				...
				if (m) {
					...
					if (!force && isDir(sdkDir)) {
						...
						if (subject === 'latest' && name === latest.name) {
							throw new CLIError(
								`Titanium SDK ${name.cyan} is already installed!`,
								`You're up-to-date. Version ${latest.name.cyan} is currently the newest version available.\n${tip}`
							);
						}
						throw new CLIError(`Titanium SDK ${name.cyan} is already installed!`, tip);
					}
				}
			}
		});
		...
		let src = path.join(tempDir, 'mobilesdk', osName, name);
		dest = path.join(titaniumDir, 'mobilesdk', osName, name);
			...
			zipfile
				.on('entry', entry => {
					...
					if (!entry.fileName.endsWith('.zip') || isSymlink || isDir) {
						return abort(new Error('Invalid SDK: does not contain a SDK zip file'));
					}
