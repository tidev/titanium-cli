...
exports.run = function (logger, config, cli, finished) {
	...
	if (SdkSubcommands[action]) {
		SdkSubcommands[action].fn.call(SdkSubcommands[action].fn, logger, config, cli, finished);
	} else {
...
async function list(logger, config, cli) {
	...
	// Gather branches?
	tasks.push(cli.argv.branches ? errorAsResult(fetch(urls.branches, config)) : Promise.resolve());
	...
	// Note that we wrapped errors to treate as "success" above so we can place errors for given sub-sections in the output
...
async function install(logger, config, cli) {
	...
	request.version && (analyticsPayload.version = request.version);
	request.branch && (analyticsPayload.branch = request.branch);
...
async function doBranch(logger, config, cli, version, branch, osName) {
	branch = await getBranch(branch, config);
	...
	if (!build) {
		...
		let str = __('Build "%s" does not exist', version) + '\n';
 ...
 */
async function getBranch(branch, config) {
	const data = await fetch(urls.branches, config);
	...
	if (!data.branches.includes(branch)) {
		let str = __('Branch "%s" does not exist', branch) + '\n';
...
async function doReleases(logger, config, cli, version, osName) {
	...
	const releases = await getReleases(config, osName);
	...
	if (!matchingBuild) {
		let str = __('Release "%s" does not exist', version) + '\n';
...
async function scanBranches(logger, config, cli, version, osName) {
	const data = await getBranches(config);
 ...
 */
async function getBranches(config) {
	return fetch(urls.branches, config);
}
...
class LocalSDKInstallRequest extends SDKInstallRequest {
	...
	async isInstalled(sdks) {
		...
		const entries = Object.entries(sdks);
 ...
 */
async function getReleases(config, os) {
	const data = await fetch(urls.releases, config);

	const releases = new Map();
	data.releases.filter(r => r.os === os && r.name === 'mobilesdk').forEach(r => {
		releases.set(r.version, r);
	});
	return releases;
}
	...
	return new Promise((resolve, reject) => {
		request({
			...
			if (response.statusCode !== 200) {
				return reject(new Error(__('Request failed with HTTP status code %s %s', response.statusCode, http.STATUS_CODES[response.statusCode] || '')), null);
			}
