...
SdkSubcommands.install = {
	...
	fn: function install(logger, config, cli, finished) {
		...
		function doReleases(callback) {
			...
			getReleases(config, osName, function (err, releases) {
				...
				if (!releases[version]) {
					...
					logger.log(cli.renderColumns(rels, '    ', 100).cyan);
					process.exit(1);
				}
		...
		function doBranch(version, callback) {
			...
			fetch(urls.branches, config, function (err, data) {
				...
				if (data.branches.indexOf(branch) == -1) {
					...
					logger.log(cli.renderColumns(data.branches.sort().reverse(), '    ', 100).cyan);
					process.exit(1);
				}
				...
				getBranchBuilds(config, branch, osName, function (err, builds) {
					...
					if (!build) {
						...
						logger.log(cli.renderColumns(buildNames, '    ', 100).cyan);
						process.exit(1);
					}
		...
		function downloadSDK(url, callback) {
			...
			req.on('response', function (req) {
				if (req.statusCode >= 400) {
					...
					if (config.get('cli.progressBars', true)) {
		...
		function extractSDK(filename, keepFiles, version, callback) {
			...
			if (!version) {
				// need to examine the zip file to figure out what version it is
				for (i = 0; i < len; i++) {
					m = zipEntries[i].entryName.match(/^mobilesdk\/[^\/]+\/([^\/]+)\/(version\.txt|manifest\.json)$/);
					if (m) {
						version = m[1];
						// maybe someday we will have a need to do something with the manifest
						break;
					}
				}
			}
				...
				if (i < len) {
					try {
						...
						if (entry.isDirectory) {
							Utils.makeDir(afs.resolvePath(installLocation, entry.entryName.toString()));
						} else {
							var content = entry.getData();
							if (!content) {
								throw Utils.Errors.CANT_EXTRACT_FILE + '2';
							}
							Utils.writeFileTo(afs.resolvePath(installLocation, entry.entryName.toString()), content, true);
						}
						setTimeout(extractFile, 0);
					} catch (ex) {
						keepFiles || fs.unlinkSync(filename);
						logger.log();
						logger.error(__('Titanium SDK %s failed to unzip.', version.cyan) + '\n');
						logger.log(error);
						process.exit(1);
					}
				} else {
		...
		async.series([
			function (next) {
				if (version) {
					...
					} else if (/.+\.zip$/.test(version) && afs.exists(version) && fs.statSync(version).isFile()) {
						// version is a file
						next(null, { file: version });
					} else {
			...
			var setDefault = (data.setDefault || cli.argv.default) && appc.version.gte(data.version, '3.0.0') && (cli.argv.default || appc.version.gt(data.version, config.get('sdk.selected', config.get('app.sdk')))),
				analyticsPayload = {};
	...
	fn: function uninstall(logger, config, cli, finished) {
		...
		async.series([
			...
			if (config.get('sdk.selected', config.get('app.sdk')) == version) {
				...
				config.save();
				logger.log(__('Updated active Titanium SDK to %s', config.sdk.selected.cyan) + '\n');
			}
...
function fetch(url, config, callback) {
	request({
		...
		if (error) {
			...
			callback(new Error(__('Request failed with HTTP status code %s %s', req.statusCode, http.STATUS_CODES[req.statusCode] || '')), null);
		} else {
			try {
