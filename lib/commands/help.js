...
exports.run = function (logger, config, cli, finished) {
	if (!afs.exists(require('../config').getConfigPath())) {
		...
		logger.log(__("Run '%s' to configure the Titanium CLI.", 'titanium setup').split('titanium setup').map(function (s) { return s.magenta; }).join('titanium setup'.cyan) + '\n');
	}
	...
	if (command && !cli.globalContext.commands[command]) {
		...
		string.suggest(command, Object.keys(cli.globalContext.commands), logger.log);
