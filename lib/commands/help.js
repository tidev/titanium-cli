				...
				Object.keys(args)
					.map(function (arg) {
						...
						args[arg].forEach(function (i, j) {
							...
							a.push([
								j > 0 ? n.replace(/./g, ' ') : n,
								(i.desc ? i.desc + ' ' : '') + (i.sdk ? ' [--sdk ' + i.sdk + (i.platform ? ' ' + '--platform ' + i.platform : '') + ']' : '')
							]);
						});
						return a;
					})
						...
						.map(function (name) {
							...
							_flags[name].forEach(function (flag, j) {
								var n = (flag.abbr ? '-' + flag.abbr + ', ' : '') + '--' + name + (flag.negate ? ', --no-' + name : '') + (flag.alias ? '|--' + flag.alias + (flag.negate ? ', --no-' + flag.alt : '') : ''),
									s = (flag.desc ? flag.desc + ' ' : '') + ((flag.default ? ' ' + __('[default: %s]', flag.default) : '') + (flag.sdk ? ' [--sdk ' + flag.sdk + (flag.platform ? ' --platform ' + flag.platform : '') + ']' : '')).grey;
