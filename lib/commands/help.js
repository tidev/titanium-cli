...
exports.run = function (logger, config, cli) {
	...
	if (command && command != '__global__' && cli.cmds[command]) {
		...
		printList(__('Commands:'), Object.keys(cli.cmds)
			.filter(function (name) {
			...
			.map(function (name) {
				var c = cli.cmds[name],
					p = c.__global__ && c.__global__.modulePath,
					items = [],
					cmd,
					conf;
				...
				try {
					...
					Object.keys(c).forEach(function (sdk) {
						if (sdk != '__global__') {
							var p = p = c[sdk].__global__ && c[sdk].__global__.modulePath;
							if (p) {
								cmd = require(p);
								conf = (cmd.config && cmd.config(logger, config, cli)) || {};
								items.push({
									...
									desc: conf.desc || '',
									params: ' [--sdk ' + sdk + ']'
									// params: ' [--sdk ' + sdk + (opt.platform ? ' --platform ' + opt.platform : '') + ']' : ''
								});
							}
						}
					});
