				...
				Object.keys(options)
					.map(function (name) {
						...
						options[name].forEach(function (opt, j) {
							...
							if (!config.cli.colors && opt.default) {
		...
		if (subcmd) {
			...
			for (var i = 0; !desc && i < orderedSDKs.length; i++) {
				if (orderedSDKs[i] == '__global__') {
					desc = cmd[orderedSDKs[i]].extendedDesc || cmd[orderedSDKs[i]].desc;
				} else {
					var platforms = Object.keys(cmd[orderedSDKs[i]]).sort();
					for (var j = 0; !desc && j < platforms.length; j++) {
						desc = cmd[orderedSDKs[i]][platforms[j]].extendedDesc || cmd[orderedSDKs[i]][platforms[j]].desc;
					}
				}
			}
