...
ModuleSubcommands.list = {
	...
	fn: function list(logger, config, cli, finished) {
		...
		if (afs.exists(p)) {
			while (path.dirname(p) != p) {
				if (afs.exists(path.join(p, 'tiapp.xml'))) {
					afs.exists(p = path.join(p, 'modules')) && searchPaths.project.push(p);
					break;
				}
				p = path.dirname(p);
			}
		...
		sdkLocations.forEach(function (p) {
			afs.exists(p = afs.resolvePath(p, 'modules')) && searchPaths.project.indexOf(p) == -1 && searchPaths.config.indexOf(p) == -1 && searchPaths.global.indexOf(p) == -1 && (searchPaths.global.push(p));
		});

		appc.timodule.scopedDetect(searchPaths, null, function (results) {
