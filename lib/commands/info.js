...
var appc = require('node-appc'),
	...
	typesList = ['all', 'os', 'nodejs', 'titanium', 'ios', 'jdk', 'android'],
	indent = 25,
		...
		function tisdk(next) {
			if (types.all || types.titanium) {
				...
				Object.keys(env.sdks).forEach(function (ver) {
					...
					try {
						v.nodeAppcVer = require(path.join(data.titanium[ver].path, 'node_modules', 'node-appc', 'package.json')).version;
					} catch (e) {}
				});
		...
		function ios(next) {
			if (process.platform === 'darwin' && (types.all || types.ios)) {
				if (!cli.argv.legacy && cli.sdk) {
					...
					var tisdk = cli.sdk.manifest && cli.sdk.manifest.version || cli.sdk[i],
						file;
...
function printData(logger, config, data, types) {
	...
	if (types.all || types.jdk) {
		logger.log(__('Java Development Kit').bold);
		if (data.jdk.version) {
