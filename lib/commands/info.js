...
function printAndroid(logger, config, data) {
		...
		if (emus.length) {
			logger.log(emus.map(function (emu) {
				return emu.name.grey + '\n' +
					...
					'  ' + rpad(__('Based On'))    + ' = ' + cyan(emu['based-on'] ? __('Android %s (API level %s)', emu['based-on']['android-version'], emu['based-on']['api-level']) : __('unknown'));
			}).join('\n') + '\n');
		} else {
			logger.log('  ' + __('None').grey + '\n');
		}
	...
	logger.log(__('Android Devices').bold);
