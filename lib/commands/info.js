...
var appc = require('node-appc'),
	...
	typesList = ['all', 'os', 'nodejs', 'titanium', 'osx', 'jdk', 'haxm', 'genymotion'];
...
exports.run = function (logger, config, cli, finished) {
	...
	// genymotion info
	if (cli.sdk && config.get('genymotion.enabled')) {
		var genymotion = path.join(cli.sdk.path, 'cli', 'lib', 'genymotion.js');
		if ((types.all || types.genymotion) && fs.existsSync(genymotion)) {
			sections.push(new Section(require(genymotion)));
		}
	}
