		...
		if (cli.argv.output == 'json') {
			...
			if ((types.all || types.titanium) && data.titanium) {
				sections['Titanium CLI'] = [
					['CLI Version', data.titaniumCLI.version],
					['node-appc Version', data.titaniumCLI.nodeAppcVer || 'unknown']
				];
				...
				Object.keys(data.titanium).forEach(function (ver) {
					...
					titanium[ver] = [
						...
						['node-appc Version', appc.version.lt(ver, '3.0.0') ? 'n/a' : x.nodeAppcVer || 'unknown']
					];
				});
			}
