		...
		if (cli.argv.output == 'json') {
			...
			if (process.platform == 'darwin') {
				...
				sections['iOS Certificates'] = [
					['Development', data.iosCerts.devNames.length ? data.iosCerts.devNames : 'not found'],
					['Distribution', data.iosCerts.distNames.length ? data.iosCerts.distNames : 'not found'],
					...
					if (target.type === "Add-On") {
						...
						if (target['libraries']) {
							...
							Object.keys(target['libraries']).forEach(function(library) {
								libraries.push(library + ': ' + target['libraries'][library].description + 
									' (' + target['libraries'][library].jar + ')');
							});
						}
					}
				});
