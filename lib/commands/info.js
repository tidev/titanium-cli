...
exports.run = function (logger, config, cli, finished) {
		...
		render: function (logger, config, rpad, styleHeading, styleValue, styleBad) {
			if (this.data) {
				...
				logger.log(
					...
					'  ' + rpad(__('Name'))         + ' = ' + styleValue(os.name) + '\n' +
					'  ' + rpad(__('Version'))      + ' = ' + styleValue(os.version) + '\n' +
					'  ' + rpad(__('Architecture')) + ' = ' + styleValue(os.architecture) + '\n' +
					'  ' + rpad(__('# CPUs'))       + ' = ' + styleValue(os.numcpus) + '\n' +
					'  ' + rpad(__('Memory'))       + ' = ' + styleValue((os.memory / 1024 / 1024 / 1024).toFixed(1) + 'GB') + '\n'
				);
			}
		}
	}));
	...
	sections.push(new Section({
		...
		render: function (logger, config, rpad, styleHeading, styleValue, styleBad) {
			if (this.data) {
				logger.log(
					styleHeading(this.title) + '\n' +
					'  ' + rpad(__('Node.js Version')) + ' = ' + styleValue(this.data.node.version) + '\n' +
					'  ' + rpad(__('npm Version'))     + ' = ' + styleValue(this.data.npm.version) + '\n'
				);
			}
		}
	}));
	...
	if ((types.all || types.ios) && !sections.some(function (s) { return s.name == 'ios'; })) {
		sections.push(new Section({
			name: 'ios',
			title: 'iOS',
			detect: function (types, config, next) {
