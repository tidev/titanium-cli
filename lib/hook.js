		...
		(isDir ? fs.readdirSync(dir) : [dir]).forEach(function (filename) {
			...
			if (fs.existsSync(file) && fs.statSync(file).isFile() && jsfile.test(filename) && (!isDir || !ignore.test(path.basename(file)))) {
				try {
					...
					if (mod.id) {
						if (this.hooks.loadedIds[mod.id]) {
							this.logger.banner();
							this.logger.error(__('Detecting conflicting CLI plugin "%s"', mod.id));
							this.logger.error('  ' + this.hooks.loadedIds[mod.id].file + ' ' + (this.hooks.loadedIds[mod.id].version ? __('(version %s)', this.hooks.loadedIds[mod.id].version) : ''));
							this.logger.error('  ' + file + ' ' + (mod.version ? __('(version %s)', mod.version) : ''));
							this.logger.error(__('Remove one of the conflicting plugins, then try again'));
							this.logger.log();
							process.exit(1);
						}

						this.hooks.loadedIds[mod.id] = {
							file: file,
