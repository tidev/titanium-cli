		...
		(isDir ? fs.readdirSync(dir) : [dir]).forEach(function (filename) {
			...
			if (fs.statSync(file).isFile() && jsfile.test(filename) && (!isDir || !ignore.test(path.basename(file)))) {
				try {
					var mod = { exports: {} },
						exp;
					(new Function('require', 'module', 'exports', '__dirname', '__filename', fs.readFileSync(file).toString()))(require, mod, mod.exports, dir, filename);
					exp = mod.exports || {};

					if (!this.version || !exp.cliVersion || semver.satisfies(this.version, exp.cliVersion)) {
						mod = require(file);
