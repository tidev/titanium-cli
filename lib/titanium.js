...
(function() {
	...
	function setLocale(cfg) {
		detectLocale(function (locale) {
			...
			wrench.mkdirSyncRecursive(path.dirname(configFilePath));
			fs.writeFileSync(configFilePath, JSON.stringify(cfg, false, '\t'));
			run();
		});
	}
		...
		.then(function (logger) {
			...
			async.series([
				...
				function (finished) {
					...
					if (logger.bannerWasRendered() && enc && !/^.+\.utf\-8$/i.test(enc) && !config.cli.hideCharEncWarning) {
						enc = enc.split('.').pop();
						console.log(__('Detected terminal character encoding as "%s". Some characters may not render properly.', enc).yellow);
