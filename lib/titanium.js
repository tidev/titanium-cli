...
cli.flag('help', {
	...
	.then(function (logger) {
		...
		// find all hooks in the global modules directory
		/* FIXME: we need to scan all modules in the modules dir for hooks, not the modules dir itself
		env.os.sdkPaths.forEach(function (p) {
			cli.scanHooks(afs.resolvePath(p, 'modules', 'cli', 'hooks'));
		});
		*/
		
		// find all hooks in the global hooks path
		(function (paths) {
			if (paths) {
				Array.isArray(paths) || (paths = [paths]);
				paths.forEach(function (p) {
		...
		async.series([
			...
			function (next) {
				...
				if (!(cmd && cli.cmds[cmd] && ((cli.cmds[cmd].__global__ && cli.cmds[cmd].__global__.skipBanner) || (cli.sdk != null && cli.cmds[cmd][cli.sdk.name].__global__ && cli.cmds[cmd][cli.sdk.name].__global__.skipBanner)))) {
					logger.banner();
				}
				next();
			},
