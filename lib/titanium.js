...
function run() {
	...
	cli.on('cli:global-args-parsed', function (data, finished) {
		var sdkName = cli.argv.sdk || config.get('sdk.selected', config.get('app.sdk')),
		...
		if (!sdk && (!data.argv._.length || cli.globalContext.commands[data.argv._[0]])) {
			finished();
			return;
		}
		...
		if (!sdk) {
			...
			if (sdks.length) {
				...
				logger.log(cli.renderColumns(sdks, '    ', config.get('cli.width', 100)).cyan);
			} else {
...
// TODO: what about platform specific commands and hooks?
